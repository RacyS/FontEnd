            ...
            <div>
                ...
                <Container className="py-5">
                    <Card className="shadow border-0">
                        <Card.Header
                            className="bg-dark text-white d-flex justify-content-between align-items-center py-3">
                                ...
                                <Row className="g-4">
                                    <Col lg={5} className="text-center">
                                        <div className="p-3 border rounded bg-light mb-3 shadow-sm">
                                            <img src={previewUrl || editingItem.itemPicture} alt="preview"
                                                 className="img-fluid rounded" style={{maxHeight: '350px'}}/>
                                        </div>
                                    ...
                                    <Col lg={7}>
                                        <Form.Group className="mb-3">
                                            ...
                                            <Form.Control value={editingItem.itemName} onChange={e => setEditingItem({
                                                ...editingItem,
                                        ...
                                        <Form.Group className="mb-4">
                                            ...
                                            <Form.Select value={editingItem.itemStatus} onChange={e => setEditingItem({
                                                ...editingItem,
                                        ...
                                        <div className="detail-section p-3 rounded bg-white border shadow-sm">
                                            ...
                                            {editDetails.map((row, index) => (
                                                <Row key={index} className="mb-2 g-2 align-items-center">
                                                    <Col xs={5}>
                                                        <Form.Select value={row.key}
                                                                     onChange={(e) => handleKeyChange(index, e.target.value)}>
                                                            {headerOptions.map(opt => <option key={opt.value}
                                                                                              value={opt.value}>{opt.label}</option>)}
                                                        </Form.Select>
                                                    </Col>
                                                    <Col xs={5}>
                                                        {row.isCustom ? (
                                                            <Form.Control placeholder="ระบุ..." value={row.value}
                                                                          onChange={(e) => {
                                                                              const d = [...editDetails];
                                                            ...
                                                            <Form.Select disabled={!row.key} value={row.value}
                                                                         onChange={(e) => {
                                                                             const d = [...editDetails];
                                                                             d[index].value = e.target.value;
                                                                             setEditDetails(d);
                                                                ...
                                                                {valueOptions[row.key]?.map(v => <option key={v}
                                                                                                         value={v}>{v}</option>)}
                                                            </Form.Select>
                                                        )}
                                                    </Col>
                                                    <Col xs={2} className="text-end">
                                                        <Button variant="link" className="text-danger p-0"
                                                                onClick={() => setEditDetails(editDetails.filter((_, i) => i !== index))}>🗑</Button>
                                                    </Col>
                                                </Row>
                                            ))}
                                        </div>
                                        <div className="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                                            <Button variant="secondary" className="px-4"
                                                    onClick={() => setEditingItem(null)}>ยกเลิก</Button>
                                            <Button variant="success" type="submit"
                                                    className="px-5 shadow-sm">บันทึกการแก้ไข</Button>
                                        </div>
                                    </Col>
                                </Row>
                            </Form>
